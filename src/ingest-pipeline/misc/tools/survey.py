...
class Entity:
    ...
    def describe(self, prefix="", file=sys.stdout):
        print(f"{prefix}{self.uuid}: " f"{self.sennet_id} ", file=file)
...
class Dataset(Entity):
    ...
    def _parse_sample_parents(self):
        ...
        if other_parent_uuids:
            ...
            sample_sennet_id = samp.sennet_id if len(other_parent_uuids) == 1 else "multiple"
            parent_dataset = None
        else:
    ...
    def build_rec(self, include_all_children=False):
        ...
        (rec["parent_dataset"], rec["sample_submission_id"], rec["sample_sennet_id"]) = (
...
class Donor(Entity):
    ...
    def describe(self, prefix="", file=sys.stdout):
        print(
            f"{prefix}Donor {self.uuid}: " f"{self.sennet_id} " f"{self.submission_id} ", file=file
        )
